...
@comentario_blueprint.route('/comentarios/addcomment', methods=['GET', 'POST'])
def crearComentarioUsuario():
    if request.method == "GET":
        form = FormComentarios()
        return render_template('comentariouser.html', form=form, action='Add')
    if request.method == "POST":
        comentario = Comentario()
        comentario.id = request.form["id"]
        comentario.Pelicula = request.form["Pelicula"]
        ...
        comentario.Usuario = request.form["Usuario"]
        sql_insert_comentario(comentario)
        return redirect(url_for('pelicula_blueprint.index'))
